...
![Version](https://img.shields.io/badge/version-1.3.5-green)  ![Python](https://img.shields.io/badge/python-3.8+-green)  ![Last Commit](https://img.shields.io/github/last-commit/cyberanchor/pypassgen)  ![Issues](https://img.shields.io/github/issues/cyberanchor/pypassgen)

## ⚠️ Compatibility notice (v1.3.5)

Starting from **v1.3.5**, the password derivation algorithm was fixed to avoid repetition for long passwords and to remove modulo bias. **This means passwords generated by versions ≤ 1.3.3 will NOT match** for the same mnemonic.
...
Colors are enabled only when `stdout` is a TTY (i.e., interactive terminal). If you redirect output to a file (`> out.txt`) or pipe it, PyPassGen automatically disables ANSI color codes.


## Security note: clearing sensitive data

PyPassGen attempts to clear sensitive data (seed/key) from memory using `bytearray` overwrites as a **best effort**. Due to Python's memory model and internal copies, this cannot be guaranteed as a complete wipe.
